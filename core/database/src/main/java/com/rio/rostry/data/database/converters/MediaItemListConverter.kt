...
/**
 * Room TypeConverter for List<SimpleMediaItem> to/from JSON.
...
class MediaItemListConverter {
    ...
    @TypeConverter
    fun fromMediaItemList(mediaItems: List<SimpleMediaItem>?): String? {
        return mediaItems?.let { gson.toJson(it) }
    }

    @TypeConverter
    fun toMediaItemList(json: String?): List<SimpleMediaItem>? {
     ...
     */
    private fun parseCommaSeparatedUrls(urls: String): List<SimpleMediaItem>? {
        return urls.split(",")
            .map { it.trim() }
