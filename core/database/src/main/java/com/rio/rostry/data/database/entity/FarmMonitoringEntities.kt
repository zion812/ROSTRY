...
data class GrowthRecordEntity(
    ...
    val mediaItemsJson: String? = null,   // Structured: JSON array of SimpleMediaItem
    ...
    @Ignore
    fun getMediaItems(): List<SimpleMediaItem> {
        if (!mediaItemsJson.isNullOrBlank()) {
            try {
                val type = object : TypeToken<List<SimpleMediaItem>>() {}.type
                val items: List<SimpleMediaItem>? = Gson().fromJson(mediaItemsJson, type)
        ...
        return photoUrl?.takeIf { it.isNotBlank() }?.let {
            listOf(SimpleMediaItem(url = it, caption = "Week $week", recordType = "GROWTH", recordId = recordId))
        } ?: emptyList()
    }
}
...
data class MortalityRecordEntity(
    ...
    fun getMediaItems(): List<SimpleMediaItem> {
        ...
        return photoUrls?.split(",")?.map { it.trim() }?.filter { it.isNotBlank() }?.mapIndexed { i, url ->
            SimpleMediaItem(url = url, caption = "Photo ${i+1}", recordType = "MORTALITY", recordId = deathId)
        } ?: emptyList()
    }
}
...
data class VaccinationRecordEntity(
    ...
    fun getMediaItems(): List<SimpleMediaItem> {
        ...
        return photoUrls?.split(",")?.map { it.trim() }?.filter { it.isNotBlank() }?.mapIndexed { i, url ->
            SimpleMediaItem(url = url, caption = "Photo ${i+1}", recordType = "VACCINATION", recordId = vaccinationId)
        } ?: emptyList()
    }
}
