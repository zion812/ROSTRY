...
abstract class AppDatabase : RoomDatabase() {
    ...
    object Converters {
        ...
        @TypeConverter
        @JvmStatic
        fun fromMediaItemList(mediaItems: List<com.rio.rostry.domain.model.media.SimpleMediaItem>?): String? {
            return mediaItems?.let { Gson().toJson(it) }
        }

        @TypeConverter
        @JvmStatic
        fun toMediaItemList(json: String?): List<com.rio.rostry.domain.model.media.SimpleMediaItem>? {
            if (json.isNullOrBlank()) return null
            val type = object : com.google.gson.reflect.TypeToken<List<com.rio.rostry.domain.model.media.SimpleMediaItem>>() {}.type
            return try {
    ...
    companion object {
        ...
        // Multimedia Gallery: owner scoping, captions, notes (91 → 92)
        val MIGRATION_91_92 = object : Migration(91, 92) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `media_items` ADD COLUMN `ownerId` TEXT")
