...
import com.rio.rostry.core.navigation.CoreRoutes
...
import com.rio.rostry.core.model.DailyGoal
import com.rio.rostry.core.model.ActionableInsight
...
import com.rio.rostry.core.designsystem.components.PendingVerificationsWidget
import com.rio.rostry.core.designsystem.components.IncomingEnquiriesWidget
import com.rio.rostry.feature.farm.dashboard.feed.FeedRecommendationCard
import com.rio.rostry.feature.farm.dashboard.feed.FeedDetailSheet
import com.rio.rostry.feature.farm.dashboard.feed.QuickFeedLogDialog
import com.rio.rostry.feature.farm.dashboard.lifecycle.StageTransitionDialog
...
fun FarmDashboardScreen(
    ...
    val checklistState = remember { object {
    ...
    // Avoid referencing app/data-layer TaskEntity from this feature module.
    // TODO(modularization): introduce a domain model or feature-local model for tasks.
    var selectedTransitionTask by remember { mutableStateOf<Any?>(null) }
        ...
        SwipeRefresh(state = refreshingState, onRefresh = { viewModel.refreshData() }) {
            ...
            item {
                SmartActionCard(
                    ...
                    onVaccinationClick = { onNavigateRoute(CoreRoutes.Builders.monitoringVaccinationWithFilter("today")) },
                    onTasksClick = { onNavigateRoute(CoreRoutes.MONITORING_TASKS) },
                    onHatchingClick = { onNavigateRoute(CoreRoutes.Builders.monitoringHatching()) },
                    onOrdersClick = { onNavigateRoute(CoreRoutes.Order.MY_ORDERS_FARMER) }
                )
            }
            ...
            item {
                Card(
                    onClick = { onNavigateRoute(CoreRoutes.Monitoring.FARM_DOCUMENT) },
            ...
            item {
                Card(
                    onClick = { onNavigateRoute(CoreRoutes.Monitoring.EXPENSE_LEDGER) },
                    modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp),
                    colors = CardDefaults.cardColors(
            ...
            item {
                ...
                if (showFeedDetail && feedRecommendation != null) {
                    FeedDetailSheet(
                        ...
                        onLogFeed = {
                            // FeedDetailSheet doesn't provide amount; use suggested amount when available.
                            val amount = suggestedFeedKg ?: feedRecommendation!!.dailyFeedKg
                            viewModel.logFeed(amount)
                ...
                if (showFeedLogDialog) {
                    ...
                    QuickFeedLogDialog(
                        ...
                        onConfirm = { amount, notes ->
                            viewModel.logFeed(amount, notes)
            ...
            item {
                Row(
                    ...
                    QuickActionItem(
                        ...
                        onClick = { viewModel.navigateToModule(CoreRoutes.FarmerNav.FARM_ASSETS) }
                    )
                    QuickActionItem(
                        icon = Icons.Filled.Storefront,
                        label = "Market",
                        color = MaterialTheme.colorScheme.surfaceVariant,
                        onClick = { viewModel.navigateToModule(CoreRoutes.FarmerNav.MARKET) }
                    )
                }
            }
            
            // Farm Status Summary (Phase 2 Modularization Requirement)
            item {
                FarmStatusCard(
                    total = uiState.farmAssetCount,
                    healthy = uiState.healthyAssetsCount,
                    sick = uiState.sickAssetsCount,
                    onClick = { viewModel.navigateToModule(CoreRoutes.FarmerNav.FARM_ASSETS) }
                )
            ...
            if (uiState.verificationStatus == com.rio.rostry.domain.model.VerificationStatus.UNVERIFIED) {
                item {
                    VerificationStatusCard(
                        ...
                        onStartVerification = { onNavigateRoute(CoreRoutes.VERIFY_FARMER_LOCATION) },
                        onRetryVerification = { onNavigateRoute(CoreRoutes.VERIFY_FARMER_LOCATION) }
                    )
                }
            }

            // Digital Farm Pipeline placeholder while feature module is being decoupled.
            item {
                Card(
                    modifier = Modifier.fillMaxWidth(),
                    ...
                    Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            Text(
                                text = "Digital Farm Pipeline",
                                style = MaterialTheme.typography.titleMedium
                            )
                            Text(
                                text = "Open the digital farm module to view stage-wise assets.",
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                        TextButton(onClick = { viewModel.navigateToModule(CoreRoutes.FarmerNav.DIGITAL_FARM) }) {
                            Text("Open")
                        }
                    }
                }
                ...
                TodayTasksCard(
                        ...
                        onTaskClick = { task ->
                            ...
                            if (task.taskType == "STAGE_TRANSITION") {
                                // Disabled during modularization
                            } else {
                                val route = when (task.taskType) {
                                    "VACCINATION" -> CoreRoutes.Builders.monitoringVaccinationWithProductId(task.productId ?: "")
                                    "GROWTH_UPDATE" -> CoreRoutes.Builders.monitoringGrowthWithProductId(task.productId ?: "")
                                    "FEED_SCHEDULE" -> CoreRoutes.Builders.monitoringDailyLog()
                                    "HEALTH_CHECK" -> CoreRoutes.Builders.monitoringQuarantine("quarantine_12h")
                                    else -> CoreRoutes.Builders.monitoringTasks("due")
                                }
                                onNavigateRoute(route)
                            }
                        },
                        ...
                        onViewAllTasks = { onNavigateRoute(CoreRoutes.MONITORING_TASKS) }
                    )
                }
            }
                ...
                if (quota.quotaBytes > 0) {
                    ...
                    if (usageRatio > 0.8) {
                        ...
                        Card(
                            ...
                            onClick = { onNavigateRoute(CoreRoutes.STORAGE_QUOTA) }
                        ) {
                            Row(
                                modifier = Modifier
                                    .fillMaxWidth()
            ...
            if (paymentsToVerify.isNotEmpty()) {
                item {
                    PendingVerificationsWidget(
                        ...
                        onViewAll = { onNavigateRoute(CoreRoutes.Order.MY_ORDERS_FARMER) },
                        onVerifyPayment = { paymentId -> 
                            onNavigateRoute(CoreRoutes.Order.paymentVerify(paymentId))
                        }
                    )
                }
            }
                ...
                item {
                    IncomingEnquiriesWidget(
                        ...
                        onQuoteClick = { quoteId -> 
                            onNavigateRoute(CoreRoutes.Order.quoteNegotiation(quoteId, false))
                        }
                    )
                }
            }
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        if (uiState.recentActivity.isNotEmpty()) {
                            ...
                            if (uiState.recentActivity.size > 5) {
                                TextButton(
                                    onClick = { viewModel.navigateToModule(CoreRoutes.Builders.productsWithFilter("recent")) },
                            ...
                            Button(onClick = { viewModel.navigateToModule(CoreRoutes.Builders.productsWithFilter("recent")) }) {
                                Text("View All")
                            }
                        }
                    }
            ...
            if (harvestReadyAlerts.isNotEmpty()) {
                items(harvestReadyAlerts) { alert ->
                    HarvestTriggerCard(
                        alert = alert,
                        onSellNow = { batchId ->
                            onNavigateRoute(CoreRoutes.Builders.createListingFromAsset(batchId))
                        },
                        onDismiss = { alertId ->
            ...
            val fetcherCards = uiState.widgets.map { widget ->
                val (icon, onClick) = when (widget.type) {
                    WidgetType.DAILY_LOG -> Icons.Filled.EditNote to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringDailyLog()) }
                    WidgetType.TASKS -> Icons.Filled.Task to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringTasks("due")) }
                    WidgetType.VACCINATION -> Icons.Filled.Vaccines to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringVaccinationWithFilter("today")) }
                    WidgetType.GROWTH -> Icons.Filled.TrendingUp to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringGrowthWithFilter("growth_record")) }
                    WidgetType.QUARANTINE -> Icons.Filled.LocalHospital to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringQuarantine("quarantine_12h")) }
                    WidgetType.HATCHING -> Icons.Filled.EggAlt to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringHatching()) }
                    WidgetType.MORTALITY -> Icons.Filled.Warning to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringMortality()) }
                    WidgetType.BREEDING -> Icons.Filled.Favorite to { viewModel.navigateToModule(CoreRoutes.Builders.monitoringBreeding()) }
                    WidgetType.READY_TO_LIST -> Icons.Filled.Storefront to { 
                        if (uiState.verificationStatus == com.rio.rostry.domain.model.VerificationStatus.VERIFIED || 
                            ...
                            viewModel.navigateToModule(CoreRoutes.Builders.productsWithFilter("ready_to_list")) 
                        } else {
                    ...
                    WidgetType.NEW_LISTING -> Icons.Filled.AddCircle to { 
                        if (uiState.verificationStatus == com.rio.rostry.domain.model.VerificationStatus.VERIFIED ||
                            uiState.verificationStatus == com.rio.rostry.domain.model.VerificationStatus.PENDING) {
                            viewModel.navigateToModule(CoreRoutes.FarmerNav.CREATE)
                        } else {
                            showVerificationPendingDialog = true
                        }
                    }
                    WidgetType.TRANSFERS -> Icons.Filled.Send to { viewModel.navigateToModule(CoreRoutes.Builders.transfersWithFilter("ELIGIBLE")) }
                    WidgetType.TRANSFERS_PENDING -> Icons.Filled.PendingActions to { viewModel.navigateToModule(CoreRoutes.Builders.transfersWithFilter("PENDING")) }
                    WidgetType.TRANSFERS_VERIFICATION -> Icons.Filled.VerifiedUser to { viewModel.navigateToModule(CoreRoutes.Builders.transfersWithFilter("AWAITING_VERIFICATION")) }
                    WidgetType.COMPLIANCE -> Icons.Filled.Warning to { viewModel.navigateToCompliance() }
                    // Evidence Order widgets
                    WidgetType.INCOMING_ENQUIRIES -> Icons.Filled.Mail to { onNavigateRoute(CoreRoutes.Order.MY_ORDERS_FARMER) }
                    WidgetType.PAYMENTS_TO_VERIFY -> Icons.Filled.Payment to { onNavigateRoute(CoreRoutes.Order.MY_ORDERS_FARMER) }
                    WidgetType.ACTIVE_ORDERS -> Icons.Filled.ShoppingCart to { onNavigateRoute(CoreRoutes.Order.MY_ORDERS) }
                }
            ...
            item {
                LiveFetcherCard(
                    ...
                    onClick = { viewModel.navigateToModule(CoreRoutes.MONITORING_FARM_LOG) }
                )
            }
            ...
            item {
                LiveCalendarFetcherCard(
                    ...
                    onClick = { viewModel.navigateToModule(CoreRoutes.FarmerNav.CALENDAR) }
                )
            }
            ...
            val digitalFarmCard = FetcherCard(
                ...
                action = "View Farm",
                onClick = { viewModel.navigateToModule(CoreRoutes.FarmerNav.DIGITAL_FARM) },
                isLocked = false
            )
        ...
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = { 
                    ...
                    if (!isPending) {
                        onNavigateRoute(CoreRoutes.VERIFY_FARMER_LOCATION)
                    }
                }) {
            ...
            dismissButton = {
                if (isPending) {
                    TextButton(onClick = { onNavigateRoute(CoreRoutes.VERIFY_FARMER_LOCATION) }) {
                        Text("Check Status")
                    }
                } else {
