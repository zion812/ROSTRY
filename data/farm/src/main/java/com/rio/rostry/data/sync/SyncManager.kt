...
class SyncManager @Inject constructor(
    ...
    private suspend fun <T> withRetry(
        ...
        repeat(attempts) { attempt ->
            try {
                ...
                // Don't retry on permission errors — they won't resolve with retries
                if (t.message?.contains("PERMISSION_DENIED", ignoreCase = true) == true) {
                    throw t
                }
     ...
     */
    override suspend fun syncAll(now: Long): Resource<SyncStats> =
     ...
     */
    override suspend fun syncFarmerDigitalFarm(now: Long): Resource<SyncStats> =
        withContext(Dispatchers.IO) {
            val timedOutDomains = mutableListOf<String>()
    ...
    private suspend fun processOutbox(ctx: SyncContext): SyncResult {
        ...
        try {
            ...
            for ((entityType, entries) in groupedEntries) {
                _syncProgressFlow.emit(SyncProgress(totalPending, processedCount, entityType, errors.toList()))
     ...
     */
    override suspend fun detectConflicts(): List<SyncConflict> = withContext(Dispatchers.IO) {
