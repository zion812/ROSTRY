...
import com.rio.rostry.data.social.mapper.toDomainModel
...
class MediaGalleryRepositoryImpl @Inject constructor(
    ...
    override suspend fun getMediaItem(mediaId: String): Result<MediaItem?> = withContext(Dispatchers.IO) {
        try {
            val entity = mediaItemDao.getMediaById(mediaId) ?: return@withContext Result.Success(null)
            val tags = mediaTagDao.getTagsForMedia(mediaId)
            Result.Success(entity.toDomainModel(tags))
        } catch (e: Exception) {
            Result.Error(e)
        }
    }
    ...
    override suspend fun getDistinctTagValues(tagType: String): Result<List<String>> = withContext(Dispatchers.IO) {
        try {
            Result.Success(mediaTagDao.getDistinctTagValues(tagType))
        } catch (e: Exception) {
            Result.Error(e)
        }
    }
    ...
    override suspend fun getMediaFile(mediaItem: MediaItem): Result<File> = withContext(Dispatchers.IO) {
        ...
        if (cachedFile != null) {
            ...
            Result.Error(Exception("File not available locally"))
        }
    }
    ...
    override suspend fun synchronizeCache(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            mediaCacheManager.enforceLruPolicy(100 * 1024 * 1024)
            Result.Success(Unit)
        } catch (e: Exception) {
            Result.Error(e)
        }
