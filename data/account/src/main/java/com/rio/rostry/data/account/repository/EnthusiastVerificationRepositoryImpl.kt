...
import com.rio.rostry.data.account.mapper.toEnthusiastVerification
...
/**
 * Implementation of EnthusiastVerificationRepository for enthusiast verification operations.
 */
@Singleton
class EnthusiastVerificationRepositoryImpl @Inject constructor(
    ...
    override fun getVerificationStatus(userId: String): Flow<EnthusiastVerification?> {
        return verificationDao.getLatestVerificationForUser(userId)
            .map { it?.toEnthusiastVerification() }
    }
    ...
    override suspend fun submitVerification(
        ...
        val entity = EnthusiastVerificationEntity(
            ...
            status = com.rio.rostry.domain.model.VerificationStatus.PENDING,
            submittedAt = now,
            reviewedAt = null,
            reviewedBy = null,
        ...
        if (user != null) {
            userDao.upsertUser(user.copy(
                verificationStatus = com.rio.rostry.domain.model.VerificationStatus.PENDING,
        ...
        onFailure = { Result.Error(it as? Exception ?: Exception(it.message)) }
    )

    override suspend fun getVerificationHistory(userId: String): List<EnthusiastVerification> {
        return verificationDao.getAllVerificationsForUser(userId)
            .map { it.toEnthusiastVerification() }
    }

    override fun getPendingVerifications(): Flow<List<EnthusiastVerification>> {
        return verificationDao.getPendingVerifications()
            .map { list -> list.map { it.toEnthusiastVerification() } }
    }
    ...
    override suspend fun reviewVerification(
        ...
        val status = if (approved) 
            com.rio.rostry.domain.model.VerificationStatus.VERIFIED 
        else 
            com.rio.rostry.domain.model.VerificationStatus.REJECTED
        ...
        if (approved) {
            ...
            if (user != null) {
                userDao.upsertUser(user.copy(
                    verificationStatus = com.rio.rostry.domain.model.VerificationStatus.REJECTED
                ))
            }
        }
