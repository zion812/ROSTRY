...
import com.rio.rostry.data.monitoring.mapper.FarmVerificationMapper
import com.rio.rostry.domain.model.VerificationStatus
import com.rio.rostry.domain.monitoring.repository.FarmVerificationRepository
...
/**
 * Implementation of FarmVerificationRepository with Firebase integration.
 * 
...
class FarmVerificationRepositoryImpl @Inject constructor(
    ...
    override fun getVerificationStatus(farmerId: String): Flow<FarmVerification?> {
        return verificationDao.getLatestVerificationForFarmer(farmerId).map { entity ->
            entity?.let { FarmVerificationMapper.toDomain(it) }
        }
    }
    ...
    override suspend fun submitVerification(
        ...
        val user = userDao.getUserById(farmerId).firstOrNull()
        if (user != null) {
    ...
    override suspend fun getVerificationHistory(farmerId: String): List<FarmVerification> {
        return verificationDao.getAllVerificationsForFarmer(farmerId).map { 
            FarmVerificationMapper.toDomain(it) 
        }
    }
}
