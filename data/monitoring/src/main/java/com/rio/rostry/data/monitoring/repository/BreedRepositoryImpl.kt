...
import com.rio.rostry.data.monitoring.mapper.BreedMapper
...
/**
 * Implementation of BreedRepository using Room database.
 * 
...
class BreedRepositoryImpl @Inject constructor(
    ...
    override fun getAllBreeds(): Flow<List<Breed>> {
        return breedDao.getAllBreeds().map { entities ->
            entities.map { BreedMapper.toDomain(it) }
        }
    }

    override fun getBreedsByCulinaryProfile(profile: String): Flow<List<Breed>> {
        return breedDao.getBreedsByCulinaryProfile(profile).map { entities ->
            entities.map { BreedMapper.toDomain(it) }
        }
    }

    override fun getBreedsByDifficulty(difficulty: String): Flow<List<Breed>> {
        return breedDao.getBreedsByDifficulty(difficulty).map { entities ->
            entities.map { BreedMapper.toDomain(it) }
        }
    }

    override suspend fun getBreedById(breedId: String): Breed? {
        return breedDao.getBreedById(breedId)?.let { BreedMapper.toDomain(it) }
    }
