...
class ProductMarketplaceRepositoryTest {
    ...
    fun `createProduct with imageBytes uploads images and returns HTTP URLs`() = runBlocking {
        ...
        // Mock file creation for each image upload
        ...
        // Verify that the product was upserted with HTTPS URLs only
        coVerify {
            productDao.upsert(
                match<ProductEntity> { entity ->
                    ...
                    entity.imageUrls.distinct().size == entity.imageUrls.size // Check distinct is applied
                }
            )
        }
    }
    ...
    fun `createProduct enforces 1_5MB size limit after compression`() = runBlocking {
        ...
        // Mock file operations for error handling
    ...
    @Test
    fun `createProduct returns only remote URLs without data prefixes`() = runBlocking {
        // Arrange
        val product = ProductEntity(productId = "test-product", sellerId = "user-123")
        val imageBytes = listOf(ByteArray(500) { 5 })

        val uploadResult = mockk<UploadResult>().apply {
            every { downloadUrl } returns "https://firebasestorage.googleapis.com/v0/b/test.appspot.com/o/products%2Ftest-product%2Fimage_0.jpg?alt=media"
        }

        coEvery { firebaseStorageUploader.uploadFile(any<String>(), any<String>(), any<Boolean>(), any<Long>()) } returns uploadResult

        // Mock file creation
        val tempFile = mockk<File>(relaxed = true)
        every { tempFile.delete() } returns true
        every { tempFile.exists() } returns true
        every { tempFile.writeBytes(any()) } just Runs
        every { mockCacheDir.resolve(any<String>()) } returns tempFile
        ...
        coVerify {
            productDao.upsert(
                match<ProductEntity> { entity ->
                    entity.imageUrls.none { url -> url.startsWith("data:") }
                }
            )
        }
    }
    ...
    @Test
    fun `createProduct applies distinct to image URLs`() = runBlocking {
        // Arrange
        val productWithDuplicates = ProductEntity(
            productId = "test-product", 
            sellerId = "user-123", 
            imageUrls = listOf("url1", "url2", "url1") // duplicate "url1"
        )
        val imageBytes = listOf(ByteArray(100) { 1 })
        ...
        coEvery { firebaseStorageUploader.uploadFile(any(), any(), any(), any()) } returns uploadResult
        ...
        // Act
        val result = repository.createProduct(productWithDuplicates, imageBytes)
        ...
        coVerify {
            productDao.upsert(
                match<ProductEntity> { entity ->
                    // Should have 3 total URLs but only 2 distinct ones (url1, url2, uploaded_url)
                    // After distinct, should be 3 URLs: 2 distinct from product + 1 from upload
                    entity.imageUrls.distinct().size == entity.imageUrls.size
                }
            )
        }
    }
