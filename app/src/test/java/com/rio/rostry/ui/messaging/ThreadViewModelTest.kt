...
private class FakeOrderRepository : com.rio.rostry.data.repository.OrderRepository {
    override suspend fun upsert(order: com.rio.rostry.data.database.entity.OrderEntity) {}
    override fun getOrderById(orderId: String): Flow<com.rio.rostry.data.database.entity.OrderEntity?> = flowOf(null)
    override fun getOrdersByBuyer(buyerId: String): Flow<List<com.rio.rostry.data.database.entity.OrderEntity>> = flowOf(emptyList())
    override fun getOrdersBySeller(sellerId: String): Flow<List<com.rio.rostry.data.database.entity.OrderEntity>> = flowOf(emptyList())
    override suspend fun softDelete(orderId: String) {}
    override suspend fun updateOrderStatus(orderId: String, newStatus: String): Resource<Unit> = Resource.Success(Unit)
    override fun getOrdersForNotification(userId: String, statuses: List<String>): Flow<List<com.rio.rostry.data.database.entity.OrderEntity>> = flowOf(emptyList())
    override fun getRecentOrdersForUser(userId: String, limit: Int): Flow<List<com.rio.rostry.data.database.entity.OrderEntity>> = flowOf(emptyList())
}
...
class ThreadViewModelTest {
    ...
    fun bind_marksThreadSeen_withCurrentUser() = runBlocking {
        ...
        val vm = ThreadViewModel(repo, dao, FakeUserProvider("u_9"), FakeOrderRepository())
        vm.bind("t1")
    ...
    fun sendQueuedDmSelf_usesCurrentUserProvider() = runBlocking {
        val repo = FakeMessagingRepo()
        val dao = FakeOutgoingDao()
        val vm = ThreadViewModel(repo, dao, FakeUserProvider("me_1"), FakeOrderRepository())
