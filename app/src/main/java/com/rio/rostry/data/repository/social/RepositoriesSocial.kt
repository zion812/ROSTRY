...
class SocialRepositoryImpl @Inject constructor(
    ...
    private val rateLimits = mutableMapOf<String, Long>() // key: action:userId -> lastTs
    private fun checkRateLimit(action: String, userId: String, windowMs: Long) {
        val key = "$action:$userId"
        val now = System.currentTimeMillis()
        val last = rateLimits[key] ?: 0L
        if (now - last < windowMs) throw IllegalStateException("Rate limited: $action")
        rateLimits[key] = now
    }
